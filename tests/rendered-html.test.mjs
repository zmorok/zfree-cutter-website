...
test("exports the ZFree Cutter landing page", async () => {
  ...
  assert.match(html, /Download v0\.6\.2/);
  assert.match(html, /Precise ±1 \/ ±5 second jumps/);
  assert.match(html, /Animated crop keyframes/);
  assert.match(html, /search and favorites now share one expandable header/);
  ...
  assert.match(html, /releases\/tag\/v0\.6\.2/);
  assert.doesNotMatch(
    html,
